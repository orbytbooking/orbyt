-- ========================================
-- MIGRATION: UPDATE PROVIDER EARNINGS TRIGGER FOR BOOKING WAGE
-- ========================================
-- IMPORTANT: Run migration 012_add_provider_wage_to_bookings.sql FIRST
-- This trigger will fail if provider_wage columns don't exist
-- 
-- If you get "record 'new' has no field 'provider_wage'" error,
-- it means migration 012 hasn't been run yet. Run it first, then re-run this migration.

-- Drop existing trigger if it exists
DROP TRIGGER IF EXISTS trigger_calculate_provider_earnings ON public.bookings;

-- Create the trigger function (will only work if provider_wage columns exist)
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND NEW.status = 'completed' AND OLD.status != 'completed' AND NEW.provider_id IS NOT NULL THEN
    ...
    -- Priority 1: Check if booking has provider_wage override
    -- NOTE: This will fail if provider_wage columns don't exist - run migration 012 first!
    ...
    -- Priority 2: Use provider's default pay rate if booking wage wasn't used
    IF NOT used_booking_wage THEN
      ...
      IF pay_rate.id IS NOT NULL THEN
        ...
        END IF;
      ELSE
        -- Priority 3: Default 80/20 split
    ...
    INSERT INTO public.provider_earnings (
      ...
      CASE 
        WHEN pay_rate_type = 'percentage' AND used_booking_wage THEN NEW.provider_wage
        WHEN pay_rate_type = 'percentage' AND pay_rate.id IS NOT NULL THEN pay_rate.percentage_rate
        ELSE NULL
      END,
      commission_amount,
      net_amount,
...
'Calculates provider earnings when booking is completed. Requires provider_wage columns (run migration 012 first).';
