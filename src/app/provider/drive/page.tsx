...
const ProviderDrive = () => {
  ...
  useEffect(() => {
    const savedFiles = localStorage.getItem("providerDriveFiles");
    if (savedFiles) {
      ...
      localStorage.setItem("providerDriveFiles", JSON.stringify(defaultFiles));
    }
  }, []);
  ...
  useEffect(() => {
    if (files.length > 0) {
      localStorage.setItem("providerDriveFiles", JSON.stringify(files));
    }
  }, [files]);
