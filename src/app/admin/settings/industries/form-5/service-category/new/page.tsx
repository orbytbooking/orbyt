...
export default function ServiceCategoryNewPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      router.push(
        `/admin/settings/industries/${formSegment}/service-category?industry=${encodeURIComponent(industry)}${scopeQs}`,
      );
    } catch (error) {
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-6 flex gap-2 justify-end">
            <Button 
              variant="outline" 
              onClick={() => router.push(`/admin/settings/industries/${formSegment}/service-category?industry=${encodeURIComponent(industry)}`)}
            >
              Cancel
            </Button>
            <Button 
