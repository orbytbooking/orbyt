...
export default function FrequencyNewPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      router.push(
        `/admin/settings/industries/form-4/frequencies?industry=${encodeURIComponent(industry)}${scopeQs}`,
      );
    } catch (error) {
        ...
        <CardContent>
          ...
          <div className="mt-6 flex gap-2 justify-end">
            <Button
              ...
              onClick={() =>
                router.push(
                  `/admin/settings/industries/form-4/frequencies?industry=${encodeURIComponent(industry)}${industryId ? `&industryId=${industryId}` : ""}${scopeQs}`,
                )
              }
            >
              Cancel
