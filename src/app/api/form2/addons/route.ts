...
import { FORM2_ADDON_SCOPE, normalizeForm2AddonWritePayload } from "@/lib/form2AddonsApi";
...
/** Form 2 add-ons only — reads/writes `industry_form2_addons`, never Form 3. */
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tenant.ok) {
      return NextResponse.json({ error: "Add-ons not found" }, { status: 404 });
    }

    const addons = await extrasService.getExtrasByIndustry(industryId, {
      businessId,
      bookingFormScope: FORM2_ADDON_SCOPE.bookingFormScope,
      listingKind: FORM2_ADDON_SCOPE.listingKind,
    });

    return NextResponse.json({ addons, extras: addons });
  } catch (error) {
    console.error("Error fetching Form 2 add-ons:", error);
    return NextResponse.json({ error: "Failed to fetch Form 2 add-ons" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const extraData = normalizeForm2AddonWritePayload(
    ...
    const addon = await extrasService.createExtra(extraData);
    return NextResponse.json({ addon, extra: addon }, { status: 201 });
  } catch (error: unknown) {
    console.error("Error creating Form 2 add-on:", error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updateData = normalizeForm2AddonWritePayload(
    ...
    const addon = await extrasService.updateExtra(id, updateData, {
    ...
    console.error("Error updating Form 2 add-on:", error);
    const { userMessage, code, details, hint } = supabaseErrorPayload(error);
    return NextResponse.json(
      { error: userMessage, details: details ?? hint ?? undefined, code: code || undefined },
      { status: 500 },
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error("Error deleting Form 2 add-on:", error);
    return NextResponse.json({ error: "Failed to delete Form 2 add-on" }, { status: 500 });
  }
}
