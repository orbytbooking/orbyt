...
/** Remove logged contract invite emails for a prospect on this form (pending "Email sent" rows). */
export async function DELETE(request: NextRequest, ctx: RouteCtx) {
  try {
    ...
    if ((formRow as { form_kind?: string }).form_kind !== "contract") {
      return NextResponse.json({ error: "Only contract forms support invite log removal" }, { status: 400 });
    }
    ...
    console.error("contract-email-sends DELETE error:", error);
