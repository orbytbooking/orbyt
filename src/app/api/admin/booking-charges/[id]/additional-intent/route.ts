...
/**
 * Create a Stripe PaymentIntent for additional charge collection in-modal.
 * Body: { amount: number }
 */
export async function POST(
  ...
  try {
    ...
    if (b.payment_provider === "authorize_net") {
      return NextResponse.json({ error: "Additional charge card modal is only available for Stripe businesses" }, { status: 400 });
    }
        ...
        ? await stripe.paymentIntents.create(
            {
              ...
              metadata: {
                ...
                kind: "booking_additional_charge",
    ...
    console.error("[booking-charges additional-intent]", e);
